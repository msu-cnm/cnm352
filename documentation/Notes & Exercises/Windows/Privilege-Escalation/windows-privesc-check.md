...
Automated script by PentestMonkey

Does not work well on Windows XP (at least when I tried)
...
```powershell
Usage: C:\Tools\privilege_escalation\windows-privesc-check-master\windows-privesc-check2.exe (--dump [ dump opts] |
 --dumptab | --audit) [examine opts] [host opts] -o report-file-stem
...
#    At least one of these to indicate what to examine (*=not implemented)
    ...
    -F INTERESTING_FILE_FILE, --interestingfilefile=INTERESTING_FILE_FILE                                          
                    #    Changes -A behaviour.  Look here INSTEAD.  On dir per line                                                                                       
    -G, --groups    #    Groups                           
    -H, --shares    #    Shares                           
    ...
    -N, --nounreadableif
                    #    Changes -A/-f/-F behaviour.  Report only interesting files readable by untrsuted users (see -x, -X, -b, -B)
    ...
    -T PATCHFILE, --patches=PATCHFILE
                    ...
                    #    Download from http://go.microsoft.com/fwlink/?LinkID=245778 or pass 'auto' to fetch automatically
    -U, --users     #    Users
...
# Show user groups and dump output for analysis
windows-privesc-check2.exe -G --dump
```